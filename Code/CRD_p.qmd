...
crd_dfw <- crd_df %>% 
  mutate(rep=factor(rep)) %>% 
   ...
   mutate(krate_kgha = factor(krate_kgha) )%>% 
...
If in simple model studentized model is automatically given, if n't given you need use muate line code below( the vlaue will be little off, but its okay)
```{r crd_resid}
...
crd_means_all
```
if you want to run model asking nrate_kgha inside krate_kgha use "|"
...
adjust = "tukey" more restriction, if adjust = none = not more rstriction
